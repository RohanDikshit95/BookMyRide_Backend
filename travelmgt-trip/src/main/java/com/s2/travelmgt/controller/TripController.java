...
public class TripController {
	...
	@RequestMapping(value = "/cancel/{TripId}", method = RequestMethod.DELETE)
	public HttpStatus deletetrip(@PathVariable int TripId) {
		Optional<TripDetails> tripid=tripInfoRepo.findById( TripId);
		System.out.println("TripEntity is "+tripid);
	TripDetails tripdet=tripid.get();

	String drivername=tripdet.getEmpName();
	System.out.println("driver name is"+drivername);
   List<Driver> d= (List<Driver>) driverrepo.findRideDriver();
   Long id=(long) 1;
   Iterator iter = d.iterator();
   while (iter.hasNext()) {
	   
     Driver temp=(Driver) iter.next();
     
   if(temp.getEmp_name()==drivername)
   {
	   temp.setStatus(id);
	   driverrepo.save(temp);
		...
		tripInfoRepo.deleteById((int)TripId);
